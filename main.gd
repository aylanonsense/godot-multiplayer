...
func _ready() -> void:
	...
	if (LaunchArgs.has_server_flag and LaunchArgs.server) or OS.has_feature("dedicated_server"):
		_switch_to_server_mode(port, true)
	else:
		_switch_to_client_mode(address, port, LaunchArgs.has_auto_connect_flag and LaunchArgs.auto_connect)
