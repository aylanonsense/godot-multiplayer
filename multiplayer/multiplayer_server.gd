...
func start_server(port: int) -> Error:
	...
	if result != OK:
		if _connection:
			_connection.destroy()
		_connection = null
	something_happened.emit("Server listening on address %s port %d" % [address, port])
	return result
...
func stop_server() -> void:
	if _connection:
		_connection.destroy()
	_connection = null
	something_happened.emit("Server stopped")
...
func _process(_delta: float) -> void:
	while _connection:
		...
		if event_type != ENetConnection.EventType.EVENT_NONE and event_type != ENetConnection.EventType.EVENT_RECEIVE:
		...
		match event_type:
			...
			ENetConnection.EventType.EVENT_RECEIVE:
				...
				for other_peer in _connection.get_peers():
					...
					new_packet.resize(1)
					new_packet.encode_u8(0, number + 1)
